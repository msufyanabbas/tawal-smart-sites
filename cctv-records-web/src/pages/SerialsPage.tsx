    ...
    >
      <div className="space-y-4">
        <p className="text-sm text-slate-500">
          Enter one serial number per line, or upload a CSV file (first column used).
        </p>

        <div>
          <label htmlFor={`${areaId}-area`} className="label">
            Serial Numbers{lineCount > 0 && <span className="ml-2 text-brand-600">({lineCount} entered)</span>}
          </label>
          <textarea
            id={`${areaId}-area`}
      ...
      <table className="min-w-full divide-y divide-slate-200 text-sm">
        <thead className="bg-slate-50 text-left">
          <tr>
            ...
            <th className="px-3 py-2 font-semibold text-slate-700">Serial Number</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {serials.map((s, idx) => (
            ...
            >
              ...
              <td className="px-3 py-2 font-mono text-slate-800">{s.serialNumber}</td>
...
const TabPanel: React.FC<{ tab: TabId }> = ({ tab }) => {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Delete ${ids.length} selected serial${ids.length > 1 ? 's' : ''}?`)) return;
    try {
      const result = await bulkDel.mutateAsync(ids);
      setSelected(new Set());
      toast.success(`${result.deleted} serial${result.deleted !== 1 ? 's' : ''} deleted`);
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Modals */}
      <AddSingleModal open={addOpen} tab={tab} onClose={() => setAddOpen(false)} />
      <BulkImportModal open={bulkOpen} tab={tab} onClose={() => setBulkOpen(false)} />
    </div>
  );
};

...
export const useCreateSimMutation = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: serialKeys.allSim }); },
  });
};
...
export const useBulkCreateSimMutation = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (serialNumbers: string[]) => api.bulkCreateSimSerials(serialNumbers),
    onSuccess: () => { qc.invalidateQueries({ queryKey: serialKeys.allSim }); },
  });
};
...
export const useCreateRmsMutation = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: serialKeys.allRms }); },
  });
};
...
export const useBulkCreateRmsMutation = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (serialNumbers: string[]) => api.bulkCreateRmsSerials(serialNumbers),
    onSuccess: () => { qc.invalidateQueries({ queryKey: serialKeys.allRms }); },
  });
};
